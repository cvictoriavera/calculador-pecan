        ...
        <div className="grid grid-cols-1 gap-4">
          {tiposGastosAdmin.map((tipo) => {
            ...
            const isAlreadyRegistered = existingCosts?.some(
              cost => cost.category === 'gastos-admin' && cost.details?.type === tipo.label
            );
            return (
              <button
                key={tipo.id}
              ...
              >
                ...
                <div className="flex flex-col items-center">
                  ...
                  {isAlreadyRegistered && (
                    <span className="text-[10px] text-muted-foreground mt-2 bg-secondary px-1.5 py-0.5 rounded border border-border/50 font-normal">
                      Ya registrado
                    </span>
                  )}
                </div>

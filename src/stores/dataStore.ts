...
import { getProductionsByCampaign, createProductionsByCampaign, getProductionsBatch } from '../services/productionService';
      ...
      loadAllProductions: async (campaigns: { id: number; year?: number; average_price?: string; total_production?: string; updated_at?: string; created_at?: string }[]) => {
        try {
          ...
          if (campaignIds.length > 0) {
            try {
              const batchResult = await getProductionsBatch(campaignIds);

              Object.entries(batchResult).forEach(([campaignIdStr, records]) => {
              ...
              console.error(`Error loading productions batch:`, error);
            }
          }

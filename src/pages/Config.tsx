...
const Config = () => {
  ...
  const [isUpdatingBenchmarking, setIsUpdatingBenchmarking] = useState(false);
  ...
  useEffect(() => {
    if (currentProjectId && projects.length > 0) {
      ...
      if (currentProject) {
        ...
        setAllowBenchmarking(Number(currentProject.allow_benchmarking) === 1);
      }
    }
  }, [currentProjectId, projects]);

  const handleToggleBenchmarking = async (checked: boolean) => {
    if (!currentProjectId) return;
    setAllowBenchmarking(checked);
    setIsUpdatingBenchmarking(true);
    try {
      await updateProject(currentProjectId, { allow_benchmarking: checked ? 1 : 0 });
      await loadProjects();
      toast({
        title: checked ? "Benchmarking activado" : "Benchmarking desactivado",
        description: `El campo allow_benchmarking se actualizó a ${checked ? 1 : 0}.`,
      });
    } catch (error) {
      console.error("Error updating allow_benchmarking:", error);
      setAllowBenchmarking(!checked);
      toast({
        title: "Error",
        description: "No se pudo actualizar el campo allow_benchmarking.",
        variant: "destructive",
      });
    } finally {
      setIsUpdatingBenchmarking(false);
    }
  };
          ...
          {isAdmin && (
            <div className="pt-4 border-t border-border/40 flex items-center justify-between">
              <div className="space-y-0.5">
                <Label htmlFor="allow-benchmarking" className="text-base font-medium cursor-pointer">
                  Permitir Benchmarking (Estadísticas)
                </Label>
                <p className="text-sm text-muted-foreground">
                  {allowBenchmarking
                    ? "Agregar este proyecto al análisis estadístico."
                    : "Excluir el proyecto del análisis estadístico."}
                </p>
              </div>
              <Switch
                id="allow-benchmarking"
                checked={allowBenchmarking}
                onCheckedChange={handleToggleBenchmarking}
                disabled={isUpdatingBenchmarking || !currentProjectId}
              />
            </div>
          )}
